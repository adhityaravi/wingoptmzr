...
function optimizeWing(DebugFlag)

    % --> To run the code in Debug Mode call the script with an argument
    %     eg., optimizeWing('debug'). Debug Mode displays every wing that
    %     fmicon evaluates in comparison with the initial wing.
    ...
    %% PreP
    % Checking for DebugFlag and fixing path conflict
    global DebugMode
    if nargin == 1
        DebugMode = 'on';
    else
    ...
    % Plotting Initial Wing
    PlotWing(Init, 'InitialWing');
    ...
    PlotCompare(DesVar_opt);
    PlotWing(DesVar_opt, 'OptimizedWing')
